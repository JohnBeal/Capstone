...
uvw <- data.frame(trigram = names(myTDM_trigram_comb_cum), 
                  trigramcount = myTDM_trigram_comb_cum,
                  bigram = sapply(names(myTDM_trigram_comb_cum), FUN = tri_bigram),
                  unigram = sapply(names(myTDM_trigram_comb_cum), FUN = tri_unigram),
...
uv <- data.frame(bigram = names(myTDM_bigram_comb_cum), bigramcount = myTDM_bigram_comb_cum, stringsAsFactors = FALSE)
...
uvw <- join(uvw, uv, by = "bigram", type = "inner")

## MLE 3-gram model ##

uvw <- mutate(uvw, MLE = (uvw["trigramcount"]/uvw["bigramcount"]))
names(uvw[ , 6]) <- "MLE"
