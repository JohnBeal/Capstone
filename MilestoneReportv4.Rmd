...
The number of documents and terms in the resulting TDM for each corpus is shown below:

```{r message=FALSE, cache=TRUE, warning=FALSE}
## Calculate TDMs for tokenized corpora training sets contained in myCorpora list. Remove stopwords and stem. ##
myTDM <- lapply(myCorpora, TermDocumentMatrix, control = list(stopwords = TRUE, stemming = TRUE))

## Number of Documents in corpora training set TDMs ##
sapply(myTDM, "[[", 5)

## Number of Terms in corpora training set TDMs ##
sapply(myTDM, "[[", 4)
```

Using the News corpus TDM as an example: there were `r myTDM[["News"]]$nrow` terms contained within `r myTDM[["News"]]$ncol` documents. As the histogram of term frequencies below demonstrates, the distribution is heavily skewed towards low frequencies - i.e. most terms occur only a few times and relatively few terms appear frequently in the corpus. 

``` {r eval = TRUE, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
library(slam)

df <- data.frame(frequency = sort(row_sums(as.matrix(myTDM[["News"]])), decreasing = TRUE))
...
Newsplot2 <- Newsplot2 + geom_hline(yintercept = 0.5, color = "red", size = 1) 
Newsplot2
```

For the News corpus this dictionary represents a subset of only `r length(df[df$frequency > df[closest(cumsum(df$frequency/sum(df$frequency)), 0.5), ], ])` out of the total `r myTDM[["News"]]$nrow` terms. The 20 highest frequency terms are shown below as an example:
